...
{ 
  ...
  update_bet_resp    <- httr::PUT(paste("https://api.matchbook.com/edge/rest/offers",sep=""),body=body_data,httr::set_cookies('session-token'=session_data$session_token),httr::content_type_json(),httr::accept_json(),httr::add_headers('User-Agent'='rlibnf'))  
  status_code        <- update_bet_resp$status_code  
  if(status_code==200)
  {
    content <- jsonlite::fromJSON(content(update_bet_resp, "text", "application/json", encoding="UTF-8"))
